...
class Project {
	...
	static isRootCommand() {
		...
		var hasConfigLoaded = (!cli.config);

		if (hasConfigLoaded || isRunningFromRoot) return true;
		...
		//config has loaded, not running in root, testing run in cwd project space
		var sourcePluginsFolder = path.join(cli.cwd, cli.plugins_dirname);
		try {
			fs.statSync(sourcePluginsFolder);
		} catch (e) {
	...
	static isStandalone() {
		...
		var rootPackageJSONPath = path.join(cli.root, "package.json");

		try {
			fs.statSync(rootPackageJSONPath);
			var rootPackageJSON = JSON.parse(fs.readFileSync(rootPackageJSONPath));
			if (rootPackageJSON['ntract-cli'] && rootPackageJSON['ntract-cli'].standalone) {
				return true;
			}
		} catch (e) {}
	...
	static getPackageJSON() {
		...
		if (Project.isStandalone() || Project.isRootCommand()) {
		...
		try {
			...
			fs.writeFileSync(sourcePackageJSONPath, JSON.stringify(packageJSON, null, "    "));
		}	
