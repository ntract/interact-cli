...
var npm = module.exports = {
	...
	install: function(cwd, packageJSON, dependencies, callback, noAPI) {
		...
		console.log("Installing npm plugins...");
		try {
			fs.accessSync(path.join(cwd,"package.json"), fs.W_OK);
		} catch(e) {
			console.log("Cannot write to",path.join(cwd,"package.json"),"perhaps run as sudo?");
			return;
		}
